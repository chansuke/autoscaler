...
func normalizeForK8sVMASScalingUp(templateMap map[string]interface{}) error {
	...
	for index, resource := range resources {
		...
		if ok && resourceType == nsgResourceType {
			if nsgIndex != -1 {
				...
				klog.Errorf("found 2 resources with type %s in the template. There should only be 1", nsgResourceType)
				return err
			}
			nsgIndex = index
		}
		if ok && resourceType == rtResourceType {
			if rtIndex != -1 {
				...
				klog.Warningf("found 2 resources with type %s in the template. There should only be 1", rtResourceType)
				return err
			}
			rtIndex = index
		}
	...
	if nsgIndex == -1 {
		...
		klog.Errorf("found no resources with type %s in the template. There should have been 1", nsgResourceType)
		return err
	}
	if rtIndex == -1 {
